...
//    }
    ...
    public void addText(int x, int y, FONTTYPE font, FONTMUL Xscal, FONTMUL Yscal, String text) {
        String str = new String();
        str = "TEXT " + x + "," + y + "," + "\"" + font.getValue() + "\"" + "," + 0 + ","
                + Xscal.getValue() + "," + Yscal.getValue() + "," + "\"" + text + "\"" + "\r\n";
        addStrToCommand(str);
    }

    public void add1DBarcode(int x, int y, BARCODETYPE type, int height, int wide, int narrow, READABLE readable,
                             String content) {
        String str = new String();
        str = "BARCODE " + x + "," + y + "," + "\"" + type.getValue() + "\"" + "," + height + "," + readable.getValue()
                + "," + 0 + "," + narrow + "," + wide + "," + "\"" + content + "\"" + "\r\n";
        addStrToCommand(str);
    }

    public void addQRCode(int x, int y, EEC level, int qrWidth, String code) {
        //var cmd = 'QRCODE 条码X方向起始点,条码Y方向起始点,纠错级别,二维码高度,A(A和M),旋转角度,M2（分为类型1和类型2）,S1 (s1-s8,默认s7),\"1231你好2421341325454353\"';
        String str = "QRCODE " + x + "," + y + "," + level.getValue() + "," + qrWidth + ",A," + 0 + ",M2,S1,\"" + code + "\"\r\n";
        addStrToCommand(str);
    }
